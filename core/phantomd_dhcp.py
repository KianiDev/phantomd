...
import os
import atexit
from typing import Dict, Tuple, Optional, List, Union, Any, Callable, Awaitable
...
class DHCPServer:
    ...
    def __init__(self,
                 ...
                 lease_db_path: Optional[str] = '/var/lib/phantomd/dhcp_leases.json') -> None:
        self.subnet: str = subnet
        self.netmask: str = netmask
        self.start_ip: str = start_ip
        self.end_ip: str = end_ip
        self.lease_ttl: int = int(lease_ttl)
        self.server_ip: Optional[str] = server_ip
        self.lease_db_path: Optional[str] = lease_db_path

        # normalize static leases (mac -> ip)
        self.static_leases: Dict[str, str] = {}
        ...
        self.rate_limit_rps: float = 10.0
        self.rate_limit_burst: float = 20.0
    ...
    def _normalize_mac(self, mac: Union[str, bytes, None]) -> Optional[str]:
        if mac is None:
            return None
    ...
    def allocate_for_mac(self, mac: Union[str, bytes], requested: Optional[str] = None) -> Optional[str]:
    ...
    def _maybe_async_save(self) -> None:
    ...
    async def _save_leases_async(self) -> None:
    ...
    def build_reply(self, op: int, xid: int, chaddr: bytes, yiaddr: str, msg_type: int,
                    prl: Optional[bytes] = None, include_t1t2: bool = False) -> bytes:
        htype = 1
        hlen = 6
        hops = 0
        secs = 0
        ...
        header = struct.pack('>BBBBIHH4s4s4s4s16s64s128s',
                             ...
                             ciaddr, yiaddr_b, siaddr, giaddr, ch, sname, filearea)
    ...
    def handle_discover(self, xid: int, chaddr: bytes, requested: Optional[str],
                        prl: Optional[bytes] = None) -> Optional[bytes]:
    ...
    def handle_request(self, xid: int, chaddr: bytes, requested: Optional[str],
                       prl: Optional[bytes] = None) -> Optional[bytes]:
    ...
    def _arp_conflict_sync(self, ip: str, timeout: int = 1) -> bool:
        try:
            ...
            if shutil.which('arping') is None:
                ...
                return False
            r = subprocess.run(['arping', '-c', '1', '-w', str(timeout), ip],
    ...
    def drop_privileges(self, user: str = 'nobody', group: Optional[str] = None,
    ...
    async def async_handle_request(self, xid: int, chaddr: bytes, requested: Optional[str],
                                   prl: Optional[bytes] = None) -> Optional[bytes]:
    ...
    async def start(self, bind_ip: str = '0.0.0.0', bind_port: int = 67) -> None:
        loop = asyncio.get_running_loop()
        self.loop = loop
        try:
            try:
        ...
        try:
            if bind_port < 1024 and os.geteuid() != 0:
                logger.error('Insufficient privileges to bind to port %d.', bind_port)
        ...
        transport, protocol = await loop.create_datagram_endpoint(
            lambda: DHCPProtocol(self), sock=sock
        )
...
class DHCPProtocol(asyncio.DatagramProtocol):
    ...
    def _send_packet(self, pkt: bytes, dst_ip: str, dst_port: int = 68, broadcast: bool = False) -> None:
        try:
    ...
    def datagram_received(self, data: bytes, addr: Tuple[str, int]) -> None:
        try:
            ...
            def send_reply(pkt: bytes, yiaddr: Optional[str] = None) -> None:
            ...
            if msg_type:
                ...
                if mt == DHCPDISCOVER:
                    async def _handle_discover() -> None:
                        try:
                    ...
                    async def _handle_release() -> None:
                        try:
                            macn = self.server._normalize_mac(chaddr)
                            if not macn:
                                return
...
def serve_cli(bind: str = '0.0.0.0', port: int = 6767, **kwargs: Any) -> None:
    srv = DHCPServer(
        kwargs.get('subnet', '192.168.100.0'),
        kwargs.get('netmask', '255.255.255.0'),
        kwargs.get('start_ip', '192.168.100.100'),
        kwargs.get('end_ip', '192.168.100.200'),
        lease_ttl=kwargs.get('lease_ttl', 86400),
        static_leases=kwargs.get('static_leases', {}),
...
async def _run_server_in_background(srv: DHCPServer, bind: str = '127.0.0.1', port: int = 6767) -> Tuple[asyncio.DatagramTransport, DHCPProtocol]:
    loop = asyncio.get_running_loop()
    transport, proto = await loop.create_datagram_endpoint(
        lambda: DHCPProtocol(srv), local_addr=(bind, port)
    )
    return transport, proto


def test_options_roundtrip() -> None:
...
def test_db_memory_sync(tmp_path: Any) -> None:
...
def test_discover_offer_request_ack_flow_isolated() -> None:
